...
@section('heading', 'Edit Jewellery Item')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-slate-200 p-6">
        <form method="POST" action="/jewellery-items/{{ $item->id }}" class="space-y-5" enctype="multipart/form-data">
            @csrf @method('PUT')

            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Item Name <span class="text-red-500">*</span></label>
                    <input type="text" name="item_name" value="{{ old('item_name', $item->item_name) }}" required class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="shop_id" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400">
                        ...
                        @foreach($shops as $shop)
                            <option value="{{ $shop->id }}" @selected(old('shop_id', $item->shop_id) == $shop->id)>{{ $shop->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Reference</label>
                    <input type="text" name="reference" value="{{ old('reference', $item->reference) }}" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Purchase Date</label>
                    <input type="date" name="purchase_date" value="{{ old('purchase_date', $item->purchase_date?->format('Y-m-d')) }}" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400">
                </div>
            </div>
            ...
            <div>
                ...
                <div class="flex gap-3">
                    ...
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="gold_type" value="{{ $type }}" class="sr-only peer" @checked(old('gold_type', $item->gold_type) == $type)>
            ...
            <div>
                ...
                <div class="grid grid-cols-4 gap-3">
                    <div>
                        ...
                        <input type="number" id="vori" name="vori" value="{{ old('vori', $item->vori) }}" min="0" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400 calc-trigger">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Ana <span class="text-xs text-slate-400">(0–15)</span></label>
                        <input type="number" id="ana" name="ana" value="{{ old('ana', $item->ana) }}" min="0" max="15" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400 calc-trigger">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Roti <span class="text-xs text-slate-400">(0–5)</span></label>
                        <input type="number" id="roti" name="roti" value="{{ old('roti', $item->roti) }}" min="0" max="5" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400 calc-trigger">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Points <span class="text-xs text-slate-400">(0–9)</span></label>
                        <input type="number" id="points" name="points" value="{{ old('points', $item->points) }}" min="0" max="9" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400 calc-trigger">
                    </div>
                </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="0.01" id="unit_price_per_gram" name="unit_price_per_gram" value="{{ old('unit_price_per_gram', $item->unit_price_per_gram) }}" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400 calc-trigger">
                    </div>
                    <div>
                        ...
                        <input type="number" step="0.0001" id="subtotal" name="subtotal" value="{{ old('subtotal', $item->subtotal) }}" readonly class="w-full border border-amber-200 bg-amber-50 rounded-xl px-4 py-2.5 text-sm font-semibold text-amber-700 cursor-not-allowed">
                    </div>
                </div>
            </div>

            <div>
                <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Photos <span class="normal-case font-normal text-slate-400 ml-1">(leave blank to keep existing)</span></p>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Document Photo</label>
                        @if($item->document_photo)
                            <img src="/{{ $item->document_photo }}" class="w-full h-32 object-cover rounded-xl border border-slate-200 mb-2">
                        @endif
                        <label class="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-xl px-4 py-4 cursor-pointer hover:border-amber-400 hover:bg-amber-50 transition">
                            <span id="doc-photo-name" class="text-xs text-slate-400">{{ $item->document_photo ? 'Replace photo' : 'No file chosen' }}</span>
                    ...
                    <div>
                        ...
                        @if($item->item_photo)
                            <img src="/{{ $item->item_photo }}" class="w-full h-32 object-cover rounded-xl border border-slate-200 mb-2">
                        @endif
                        <label class="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-xl px-4 py-4 cursor-pointer hover:border-amber-400 hover:bg-amber-50 transition">
                            <span id="item-photo-name" class="text-xs text-slate-400">{{ $item->item_photo ? 'Replace photo' : 'No file chosen' }}</span>
